...
class LabelSmoothSoftmaxCE(nn.Module):
    ...
    def __init__(self, lb_smooth=0.1, reduction='mean', ignore_index=-100, weight=None):
    ...
    def forward(self, logits, label):
        ...
        logs = self.log_softmax(logits) # n*23
        if self.weight is not None:
            logs = logs * self.weight * lb_one_hot
        else:
            logs = logs * lb_one_hot
            
        loss = -torch.sum(logs, dim=1)
        loss[ignore] = 0
        if self.reduction == 'mean':
            loss = loss.sum() / (n_valid + 1e-8)
