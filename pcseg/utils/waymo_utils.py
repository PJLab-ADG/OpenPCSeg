...
def generate_labels(frame, pose):
    ...
    for i in range(len(laser_labels)):
        ...
        speeds.append([laser_labels[i].metadata.speed_x,
                      laser_labels[i].metadata.speed_y])
        accelerations.append(
            [laser_labels[i].metadata.accel_x, laser_labels[i].metadata.accel_y])
    ...
    if annotations['name'].__len__() > 0:
        global_speed = np.pad(annotations['speed_global'], ((
            0, 0), (0, 1)), mode='constant', constant_values=0)  # (N, 3)
    ...
    for c in calibrations:
        ...
        for cur_ri_index in ri_index:
            ...
            cp_tensor = tf.reshape(
                tf.convert_to_tensor(cp.data), cp.shape.dims)
            cp_points_tensor = tf.gather_nd(
                cp_tensor, tf.where(range_image_mask))
        ...
        points_intensity.append(np.concatenate(
            points_intensity_single, axis=0))
        points_elongation.append(np.concatenate(
            points_elongation_single, axis=0))
...
def save_lidar_points(frame, cur_save_path, use_two_returns=True):
    ...
    points, cp_points, points_in_NLZ_flag, points_intensity, points_elongation = convert_range_image_to_point_cloud(
        frame, range_images, camera_projections, range_image_top_pose, ri_index=(
            0, 1) if use_two_returns else (0,)
    )
    ...
    points_elongation = np.concatenate(
        points_elongation, axis=0).reshape(-1, 1)
...
def process_single_sequence(sequence_file, save_path, sampled_interval, has_label=True, use_two_returns=True, update_info_only=False):
    ...
    for cnt, data in enumerate(dataset):
        ...
        info = {}
        pc_info = {'num_features': 5,
                   'lidar_sequence': sequence_name, 'sample_idx': cnt}
    ...
    print('Infos are saved to (sampled_interval=%d): %s' %
